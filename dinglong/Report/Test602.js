...
async function testreportwithloanid(){
    ...
    var reporteditem=await report.AddReport(idToken,c.reporttype1,c.reportStartDate,loanedoutloan["id"],"",c.reportitemid,c.reportquantity,c.reportstatementname,c.reportstatementalias,c.reportstatementnric,c.reportstatementage,c.reportstatementsex,c.unitid,c.rerportstatementservicerank,c.reportstatementvocation,c.reportstatementservicetype,c.reportstatementdoe,c.reportstatementord,c.reportstatement)
    
    assert.notEqual(reporteditem["id"],undefined,"User cannot add loans")

    console.log("- - - - - - - - - - - - - - - - - - - - - - - - - - ")
    console.log(`${c.manageraccount} Reported ${c.reportquantity} Item ${c.reportitemid} with loan id of ${addedloan["id"]} was lost/damaged`)
    console.log("- - - - - - - - - - - - - - - - - - - - - - - - - - ")
    ...
    assert.equal(getsinglereport["type"],reporteditem["type"],"TEST 602 FAILED : Status of loan should change when a user reports an item with a loan id given")
